...
mappertbl2:
	...
	.word mapVTinit   	@253 VT03/VT09 OneBus
...
#if VT_MODE
	@ Clear the runtime "is VT cartridge" flag.  mapVTinit will set it
	@ back to 1 if the mapper turns out to be one of ours.  Any other
	@ mapper leaves it at 0, so the PPU $2008+ divert in ppu.s falls
	@ through to standard NES mirror handling.
	ldr r0,=vt_active
	mov r1,#0
	strb r1,[r0]
#endif
...
@----------------------------------------------------------------------------
 ...
 .pool
@----------------------------------------------------------------------------
@ Runtime mirroring setter for the VT mapper (session 15).
@ r0 bit0 = the value written to $4106 (H/V scrolling selector) or to the
@ MMC3-compatibility register $A000.  In both encodings bit0 == 0 means the
@ two nametables sit SIDE BY SIDE so the game can scroll horizontally --
@ which is what the NES calls vertical mirroring (m0101).  bit0 == 1 stacks
@ them (horizontal mirroring, m0011).  Falls into the stock mirrorchange, so
@ it returns straight to our caller through its own lr handling.
	.global vt_set_mirroring
vt_set_mirroring:
	tst r0,#1
	ldreq r0,=m0101		@vertical mirroring: nametables side by side
	ldrne r0,=m0011		@horizontal mirroring: nametables stacked
	b mirrorchange
...
_twitch:	.byte 0
@ Default 0 = "No Flicker".  PocketNES's scaled mode alternates which NES
@ scanlines are dropped each frame (adjustblend/twitch in ppu.s) so a real
@ GBA LCD blends 240 source lines into 160.  On modern screens and
@ emulators the alternation reads as the whole image shaking up and down
@ by a line.  A stable skip pattern (flicker=0) looks far better there;
@ users on original hardware can still enable Flicker from the menu.
_flicker: .byte 0
@padding
	.byte 0,0
@padding:
