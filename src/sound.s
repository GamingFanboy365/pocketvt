...
#if VT_ADPCM_SOUND
	@ Mix VT ADPCM channels into the 8-bit DirectSound PCMWAV buffer.
	@ vt_adpcm_mix_gba(u8 *buf, int num_samples) adds ADPCM output
	@ (scaled from 16-bit to 8-bit) into the signed 8-bit PCMWAV buffer.
	@ r4-r8 are already on the stack (saved at start of timer1interrupt).
	bl_long vt_adpcm_mix_gba
#endif
...
frame_irq_handler:
	@ VT-family (OneBus) gate: the XOP APU behaves as if the 2A03 frame-
	@ counter IRQ does not exist for these games.  Lonely Island writes
	@ $4017 at boot, never reads $4015, and its IRQ stub acks only the VT
	@ timer ($4103) -- the stock frame IRQ therefore re-asserts forever and
	@ starves the 6502 in an interrupt storm (traced: back-to-back $FCAF
	@ entries, mainline never runs).  In VT mode, never assert IRQ_APU;
	@ envelope/length clocking continues.
	ldr r0,=vt_active
	ldrb r0,[r0]
	movs r0,r0
	bne 9f
	...
	strb_ r0,wantirq
	b_long CheckI
9:	@ gated: NOTHING asserted.  CheckI assumes its caller verified wantirq;
	@ entering it with wantirq==0 vectors a phantom IRQ through $FFFE
	@ (observed: irq6502 hit with _wantirq==0; the game stub then applied
	@ zeroed raster shadows and killed NMI).  Re-check, stock-idiom style.
	ldrb_ r0,wantirq
	movs r0,r0
	bne 7f
	b_long _GO
...
@------------------------------------------------------------------------------
@ VT timer on the cycle-accurate timeout system.  The handler reschedules
@ itself every period (scanlines, 341 dots each) and asserts the mapper IRQ
@ only when the game has enabled it ($4104) AND rendering has been seen once
@ since reset (boot-vector safety: an IRQ before the game sets its RAM
@ vector at ($003A) crashes the 6502 -- observed r9=0xFFFF8000).
@------------------------------------------------------------------------------
	global_func vt_timer_install_now
vt_timer_install_now:
	@ Called from vt_reg_write (in-core memory-handler context, r10 valid
...
vt_timer_handler:
	...
	ldr r0,=vt_timer_handler
	bl_long install_timeout_2
	@ assert only when game-enabled and boot-latched
	ldr r0,=vt_timer_render_seen
	ldrb r1,[r0]
	movs r1,r1
	beq 8f
	ldr r0,=vt
	ldrb r1,[r0,#0x6C]		@ vt.timer_ctrl
	tst r1,#0x02
	beq 8f
	mov r1,#1
	strb r1,[r0,#0x6D]		@ vt.want_timer_irq
	ldrb_ r0,wantirq
	orr r0,r0,#IRQ_MAPPER
	strb_ r0,wantirq
	b_long CheckI
8:	@ gated skip: nothing asserted -- never enter CheckI unverified
	ldrb_ r0,wantirq
	movs r0,r0
	bne 7f
	b_long _GO
