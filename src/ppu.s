...
skipdma:
	...
	bl_long run_palette
#if VT_MODE
	@ run_palette just refreshed the GBA palette from the legacy 32-byte
	@ nes_palette view (4 entries per sub-palette).  In VT 16-colour mode
	@ that layout is wrong -- the 16-entry scattered-index sub-palettes
	@ built by vt_build_16color_palette are the correct final state, so
...
@	stmfd sp!,{r3-r11,lr}
	...
	@ Rebuild GBA palette from VT extended palette RAM if dirty.
	@ Must run during VBlank when palette RAM writes are safe.
	bl_long vt_palette_rebuild_gba
#endif
...
@ 	cache_bank=map[spr_bank];
	...
	ldrb r4,[r8,r4]
#if VT_MODE
	@ SPEVA: when Sprite Address Extension is live, the sprite's CHR bank is
	@ (page_bank<<3)|EVA with EVA = OAM byte2 bits 2-4 (r3 bits 18-20).  C
	@ publishes a slot for each live (page,EVA) pair at spr_cache_map[64 +
...
@----------------------------------------------------------------------------
#if VT_MODE
	@ VT extended PPU registers are at $2008+ on VT chips, where real NES
	@ would just be mirrors of $2000-$2007.  Only divert when running an
	@ actual VT cartridge (vt_active set by mapVTinit); on any other mapper
	@ fall straight through to the standard mirror dispatch below so NES
	@ behaviour is preserved.
	ldr r2,=vt_active
	ldrb r2,[r2]
	cmp r2,#0
	bne vt_ppu_w_check_extended
#endif
PPU_W_dispatch:
	and r2,addy,#7
	ldr pc,[pc,r2,lsl#2]
...
#if VT_MODE
vt_ppu_w_check_extended:
	@ vt_active is set: check whether this is a standard PPU register
	@ ($2000-$2007) or an extended VT register ($2008+).
	sub r2,addy,#0x2000		@ offset from PPU base
	cmp r2,#8			@ standard regs are offset 0-7
	blo PPU_W_dispatch		@ offset < 8 -> standard NES PPU reg
	b vt_ppu_extended_W

vt_ppu_extended_W:
	@ r0 = value being written, addy (r12) = NES bus address.
	@ vt_ppu_reg_write(u8 page, u8 offset, u8 val)
	@   r0 = page  (0 for $20xx, 1 for $21xx)
	@   r1 = offset (low byte of address)
	@   r2 = val   (the byte being written)
	stmfd sp!,{r2,r12,lr}
	mov r2,r0			@ r2 = val (save before we clobber r0)
...
@----------------------------------------------------------------------------
	.global vmdata_W
@ ----------------------------------------------------------------------------
@ vt_pal_dma_fast -- bulk palette upload fast path (session 11)
@
@ Lonely Island uploads its ENTIRE palette every frame with the VT video DMA
@ ($4034 bit0 + $4014 trigger, 128 bytes into $2007 starting at $3F00).  The
@ generic DMA loop in mapVT.s calls vmdata_W per byte: address decode, table
@ dispatch, then VRAM_pal.  That showed up as ~11% of host time (VRAM_pal +
@ vmdata_W) in the profile.  This helper recognises the common case -- the
@ whole transfer lands inside the 128-byte palette window with increment 1 --
@ and performs the identical stores in one tight loop.
@
@ in:  r0 = source NES address, r1 = byte count
@ out: r0 = 1 if the transfer was handled here, 0 to fall back to the loop
@ clobbers r1,r2,r3,r12 (addy) only; caller keeps r4-r7.
@ ----------------------------------------------------------------------------
	.global vt_pal_dma_fast
	.global nes_palette
vt_pal_dma_fast:
	stmfd sp!,{r4-r8,lr}
	...
	add r12,r3,r1
	cmp r12,#0x80			@ must not run past $3F7F
	bhi .Lpdf_no
...
.Lpdf_skip_mirror:
	...
	ldr_ r1,vramaddr		@ advance vramaddr past the transfer
	bic r1,r1,#0x7f			@ (r3 = start offset + count, <= 0x80)
	orr r2,r1,r3
	strh_ r2,vramaddr

	ldr r1,=vt_palette_dirty
	mov r2,#1
	strb r2,[r1]
	mov r0,#1
	ldmfd sp!,{r4-r8,pc}
...
vmdata_W:	@(2007)
@----------------------------------------------------------------------------
	@ (A session-5 debug counter used to live here, incrementing a byte in
	@ EWRAM on EVERY $2007 write.  It cost a push/pop plus a slow EWRAM
	@ read-modify-write per byte of every nametable and palette upload --
	@ measurable in the session-11 profile.  Removed.)
	ldr_ addy,vramaddr
vramaddrinc_modify1:
...
@	ldr_ r2,agb_nt3
	...
	.endif
@----------------------------------------------------------------------------
VRAM_pal:	@write to VRAM palette area ($3F00-$3F1F, plus $3F80-$3F9F on VT03)
@----------------------------------------------------------------------------
	cmp addy,#0x3f00
	bmi VRAM_name3
...
#if VT_MODE
	@ VT03/VT09 palette extension (Furbtendulator OneBus.cpp:540):
	@   $3F00-$3F1F = low  6-bit half of composited colour index
	@   $3F80-$3F9F = high 6-bit half (zero on standard NES; VT03+ only)
	@ Decoded as Palette[idx|0x80]<<6 + Palette[idx|0x00] -> 4096-entry
	@ pseudo-palette indexed into PALETTE_VT03 base.
	@
	@ We branch on bit 7 of addy to dispatch lo vs hi.  Both paths additionally
	@ notify the C side so vt_palette_ram[] stays in sync with nes_palette[].
	@ Only the lo path touches nes_palette (preserving legacy NES rendering).
	@
	@ Note: this code runs even when vt_active==0 (plain NES ROM with no VT
	@ mapper).  The C bridge functions are cheap stubs that only flip the
	@ vt_palette_active flag once a real hi-byte write occurs, so non-VT carts
	@ stay on the legacy path through vt_palette_rebuild_gba's early-out.
	tst addy,#0x80
	...
	@ time -- LI rewrites its whole palette every frame.  Inline the
	@ exact same semantics: linear store of the full 7-bit offset into
	@ vt_palette_ram, legacy ^0x10 mirror confined to the low 32 bytes,
	@ dirty flag.  (vt_palette_write_lo itself remains for the $214x
	@ window path in vt_regs.c.)
	stmfd sp!,{r2}
	ldr r1,=vt_palette_ram
	and r2,addy,#0x7f
	strb r0,[r1,r2]
	cmp r2,#0x20
	bhs 1f
...
1:
	...
	strb r0,[r1,addy]		@store in nes palette
		tst addy,#0x03
		eoreq addy,addy,#0x10	@background color mirroring (high<->low)
		streqb r0,[r1,addy]
		biceq addy,addy,#0x10
	mov pc,lr

#if VT_MODE
@----------------------------------------------------------------------------
VRAM_pal_hi:	@VT03 hi-byte palette write ($3F80-$3F9F)
@----------------------------------------------------------------------------
	@ Inputs: r0 = value, addy = $3F80..$3F9F (bit 7 set).
