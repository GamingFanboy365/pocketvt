...
no4scr:
#if VT_MODE
	@ Debug/testing hook: OR a script-controlled byte into the NES pad
	@ state (bit0=A, bit1=B, bit2=Select, bit3=Start, bit4=Up, bit5=Down,
	@ bit6=Left, bit7=Right).  vt_dbg_pad_or lives in EWRAM and is 0 in
	@ normal play (zero-init BSS, never written by the emulator), so this
	@ costs one load+or per frame and lets an attached debugger inject
	@ input via memory writes for headless gameplay testing.
	ldr addy,=vt_dbg_pad_or
	ldrb addy,[addy]
	orr r0,r0,addy
#endif
	tst r2,#0x20000000
...
@ bit30 (0x40000000) SET: also store the pad into joy1state each frame.
@ Lonely Island (and, typically, other VT plug-and-play boards) wires its
@ single controller as NES PLAYER 2 -- the map-movement handler reads the
@ $4017-derived pressed-edge var ($36).  With bit30 clear the game never
@ saw any input at all ("not possible to do gameplay").  Feeding the same
@ byte to both pads is what the famiclone hardware effectively does.
_joycfg: .word 0x60ff01ff @byte0=auto mask, byte1=(saves R)bit2=SwapAB, byte2=R auto mask
